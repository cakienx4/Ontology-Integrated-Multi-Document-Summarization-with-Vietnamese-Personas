...
async def xu_ly_mot_profile_bien_the(client, semaphore, p, truong_list):
    p_loc = loc_truong(p, truong_list)
    p_loc["mo_ta_chung"] = build_mo_ta_chung(p, "Nam", truong_list=truong_list)
    async with semaphore:
        try:
            p_loc["mo_ta_chung"] = await goi_llm_async(client, tao_prompt_mo_ta_bien_the(p, truong_list))
...
async def sinh_bien_the_async(client, semaphore, truong_list, coverage_profiles, da_co=None):
    da_co = da_co or {}
    ...
    tasks = [xu_ly_mot_profile_bien_the(client, semaphore, p, truong_list) for p in con_lai]
    ket_qua_moi = await asyncio.gather(*tasks)
    ket_qua.extend(ket_qua_moi)
    return ket_qua
...
if __name__ == "__main__":
    ...
    async def chay_tat_ca():
        ...
        for truong_list in danh_sach_can_chay:
            ...
            ket_qua = await sinh_bien_the_async(client, semaphore, truong_list, coverage_profiles, da_co=da_co)
